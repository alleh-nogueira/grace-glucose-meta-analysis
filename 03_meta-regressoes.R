...
annotate("text", label = paste0("P = 0.67"), x = "Baixo", y = 0.065) +
labs(
    x = "", y = "Diferença Média na ASCROC\n",
    fill = "Risco Global de Vieses", color = "Risco Global de Vieses"
) +
scale_fill_manual(
values = c("#9C89B8", "#6C548C"),
limits = c("Baixo", "Moderado")
) +
scale_color_manual(
values = c("#9C89B8", "#6C548C"),
limits = c("Baixo", "Moderado")
...
labs(
    x = "\nPrevalência de Diabetes na Admissão",
    y = "Diferença Média na ASCROC\n"
)
...
labs(
    x = "\nPrevalência de IAMCSSST na Admissão",
    y = "Diferença Média na ASCROC\n"
)
...
labs(
    x = "\nPeríodo de Aferição da Mortalidade (Dias)",
    y = "Diferença Média na ASCROC\n"
)

pdf("figura-4.pdf", width = 12, height = 8)
